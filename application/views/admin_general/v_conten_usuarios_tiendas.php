          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Usuarios <small>de Tiendas</small></h3>
              </div>
            </div>
            <div class="row">
                 <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class=""><h2>Lista de Usuarios</h2></div>
                  <div class="x_content">
                    <p class="text-muted font-13 m-b-30">Listado de usuarios inscritos en el sistema</p>
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                     ...
                     <tbody id="showdataUsuarioTienda">
                        <?php
                             if(empty($usuarios_tienda)){
                               }else {
                                $contador =0;
                                     foreach ($usuarios_tienda as $fila): 
                               $contador ++;
                                      ?>
                        <tr>
                          <td><?= $contador;?></td>
                          <td>
                            <a><?= $fila->nombre_tienda;?></a>
                            <br />
                            <small>Created <?= $fila->fecha;?></small>
                          </td>
                          <td>
                              <?= $fila->usuario;?>
                          </td>
                          <td>
                            ...
                            <td>
                                <?php
                                 if ($fila->plan_solicitado ==0){ ?>
                                   <a id="" disabled="true"  data="" class="btn btn-default btn-xs" 
                                      title="No hay plan solicitado" data-toggle="tooltip">
                                      <?php echo "No hay plan Solicitado"; ?>
                                   </a>
                                <?php  }else{
                                 $plan_solic = $this->modelogeneral->datos_plan($fila->plan_solicitado); ?>
                                
                                <a data="<?= $fila->plan_solicitado.'-'.$fila->id_usuario; ?>" 
                                  class="btn btn-warning btn-xs  aut-plan" title="Aprobar Plan" data-toggle="tooltip"> 
                                 <?= $plan_solic->tipo_plan; ?>
                                   
                                 </a>
                               <?php } ?>
                            </td>
                            <td>
                              <a href="javascript:;" data="<?= $fila->id_usuario; ?>" class="btn btn-primary btn-xs item-view" ><i class="fa fa-eye" ></i> Ver </a>
                              <a href="javascript:;" data="<?= $fila->id_usuario; ?>"  class="btn btn-info btn-xs item-edit" data-toggle="modal" data-target=""><i class="fa fa-pencil"></i> Editar </a>
                              <a href="javascript:;" data="<?= $fila->id_usuario; ?>" class="btn btn-danger btn-xs item-delete"  ><i class="fa fa-trash-o"></i> Eliminar </a>
